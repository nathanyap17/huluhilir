...
| Class | Minimum viable | Realistic target | Notes |
|---|---|---|---|
| `healthy_leaf` | 100 | ~200 | Easy — no infected farm needed; start today |
| `foliar_yellowing` | 100 | ~200 | Matches its pair; non-specific / nutrient-deficient vines |
| `healthy_collar` | 100 | 100–130 | Match `collar_lesion`; no infected farm needed; **same framing as `collar_lesion`, disease-free** |
| `collar_lesion` | 100 | 100–130 | **The binding constraint** — needs active infection / farm access |
| `defoliation_wilt` | 80 | ~100 | Standalone, no pair partner; medium framing (§3.2) |
| `unrelated` | 50 | ~60 | Soil, hand, sky, blur — casual capture, no field visit required |
| **Total** | **~530** | **~760–820** | Before augmentation |
...
**`collar_lesion` is the only genuinely gated class.** `healthy_collar` needs no infected farm — so if farm access slips or is delayed, you can still bank `healthy_collar` immediately and only `collar_lesion` waits.

### 2.1 · The within-pair balance principle

**Why cross-domain size differences are acceptable: within-pair balance is what matters.** The model's hard job is not distinguishing a leaf from a collar — those look completely different across body parts. The model's hard job is discriminating *within* each domain: telling a healthy collar from a lesioned one, and a healthy leaf from a yellowing one. Those are the fine decision boundaries.

What matters is balance inside each pair, not across pairs:

```
LEAF DOMAIN     healthy_leaf 200    ↔  foliar_yellowing 200     ratio 1:1  ✓
COLLAR DOMAIN   healthy_collar ~100 ↔  collar_lesion ~100       ratio 1:1  ✓
```

- **Both pairs are internally balanced.** The ~2:1 gap between the leaf domain (~400 total) and collar domain (~200–260 total) is mild and harmless — classes across domains are not competing for the same fine decision boundary.
- **Class weighting handles the residual gap.** Standard inverse-frequency weighting in the loss function (§4.4) cleanly corrects for the domain size ratio. A 2:1 ratio is well within normal operating parameters (worry at 10:1).
- **Match your pair partner, not the opposite domain.** Do not inflate `healthy_collar` to 200 just to match the leaf count. The guiding rule is parity with `collar_lesion` (~100–130).
- **Uniform augmentation across classes — do NOT over-augment collars.** Do not try to close the collar-to-leaf gap by applying higher augmentation multipliers to collar images. Heavy augmentation on fewer unique source vines simply duplicates the same plants without adding new visual information, artificially inflating training scores without improving real generalization. Keep the augmentation multiplier uniform (~7×) and let loss class weighting do the balancing.

**Use the existing `capture_target` field to tag collection, not just app use.** `observations.capture_target` (`leaf` / `collar` / `whole_vine`, in `docs/DATA_MODEL.md` §9) should be set on every training image at capture time, so per-domain balance can be verified before training — not discovered after.

### 2.2 · Split & reporting caveats — stratified 70 / 15 / 15 (or per-class allocation)

```
train   70%   ~530–570 imgs   → augmented to ~3,700–4,000 (uniform ~7×)
val     15%   ~115–125 imgs   → NO augmentation
test    15%   ~115–125 imgs   → NO augmentation, touched once
```

#### The collar test-set reporting caveat
At ~100 originals with a standard 70/15/15 split, `collar_lesion` receives ~15 held-out test images. While sufficient to validate training convergence, this sample size is thin for statistical reporting:
- **High metric volatility:** Each individual test image shifts recall by ~6.7 percentage points.
- **Wide confidence intervals:** A measured recall of 0.75 (11–12 of 15) carries roughly a ±20% confidence interval — true population recall could plausibly sit between ~0.53 and ~0.90.

Since `collar_lesion` recall is the headline metric with a stated threshold of ≥ 0.75 (§6.2), handle this via three options (in order of preference):
1. **Push collar capture to ~130** if farm access allows → yields ~20 test images, tightening the interval meaningfully.
2. **Allocate a larger test share per class (e.g. 60/15/25 for collar classes)** → uneven split per domain costs very little in training data while gaining crucial statistical power on the critical evaluation metric.
3. **Report the interval honestly** → e.g. stating *"collar_lesion recall 0.78 on 15 held-out images"* is defensible; quoting 0.78 bare as an exact figure is not.
...
| `huluhilir_l1.tflite` | < 6 MB (fp16) | Flutter, `tflite_flutter` |
...
|---|---|---|
| Where | `api` container, ONNX Runtime | Phone, `tflite_flutter` |
